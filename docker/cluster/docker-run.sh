...
case "$_operation" in
    'recreate')
      echo "=>recreate, remove -> build -> create"
      docker-compose -p wildfly-cluster down
      docker images --format "{{.Repository}}:{{.Tag}}" | grep '^wildfly' | xargs -r docker rmi -f
      ...
      docker-compose -p wildfly-cluster up -d
